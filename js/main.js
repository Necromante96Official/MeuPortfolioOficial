 ...
 var linksWrap = document.querySelector('[data-detail="links"]');
  if(linksWrap){
    ...
    if(p.private) {
      ...
      links.push('<div style="display:inline-flex; flex-direction:column; align-items:center; gap:6px; padding:18px 24px; background: linear-gradient(135deg, rgba(168,85,247,0.16), rgba(168,85,247,0.06)); border:1px solid rgba(168,85,247,0.28); border-radius:14px; color:#e9d5ff; font-size:0.92rem; font-weight:800; letter-spacing:0.04em; box-shadow:0 12px 28px rgba(168,85,247,0.20); text-align:center; min-width:260px;"><span style="font-size:1.2rem;">🔒</span><span>'+privateLabel+'</span><span style="font-size:0.70rem; font-weight:600; color:var(--text-dim); margin-top:4px; display:block; text-align:center; width:100%;">Repositório privado acesso sob solicitação</span></div>');
    }
    if(!primaryHref && !p.private) {
      var privateLabel2 = dict['projects.detail.private'] || 'Código privado, protegido';
      links.push('<div style="display:inline-flex; flex-direction:column; align-items:center; gap:6px; padding:18px 24px; background: linear-gradient(135deg, rgba(168,85,247,0.16), rgba(168,85,247,0.06)); border:1px solid rgba(168,85,247,0.28); border-radius:14px; color:#e9d5ff; font-size:0.92rem; font-weight:800; letter-spacing:0.04em; box-shadow:0 12px 28px rgba(168,85,247,0.20); text-align:center; min-width:260px;"><span style="font-size:1.2rem;">🔒</span><span>'+privateLabel2+'</span><span style="font-size:0.70rem; font-weight:600; color:var(--text-dim); margin-top:4px; display:block; text-align:center; width:100%;">Repositório privado acesso sob solicitação</span></div>');
    }
